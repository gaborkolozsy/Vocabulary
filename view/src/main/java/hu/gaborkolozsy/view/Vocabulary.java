 ...
 * után <font color="green">zöld</font> színű lesz az. Máskülönben megjelenik 
 * a helyes fordítás <font color="red">pirossal</font>.<br>
 ...
 *      nyelvkombináción. Így hat megszerezhető kis csillag van 24 féle 
 *      variációban. A {@code toolTipText}-ek feliratai érdekességeket ill. 
 *      <b>Romhányi József</b> verseket tartalmaznak részletekben. 
 *      A teljes változat csak a kör teljesítésekor megkapott csillag után 
 *      olvasható.
 *  </li>
 * </ol>
 * 
...
public class Vocabulary extends JFrame {
    ...
    private void languageComboBoxActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_languageComboBoxActionPerformed
        ...
        if (startButton.getText().equals(" Start")) {
            ...
            if (from.equals(GER) || to.equals(GER)) {
                languageComboBox.setSelectedIndex(1);
            } else {
            if (from.equals(ENG) && to.equals(GER)) {
                languageComboBox.setSelectedIndex(2);
            } else {
            if (from.equals(GER) && to.equals(ENG)) {
                languageComboBox.setSelectedIndex(2);
            }}}}
        }
        ...
        if (startButton.getText().equals(" run...")) {
            if (answerTxtField.getForeground() == Color.black) {
                if (vs.getVocabularyValue(key).equals(answerTxtField.getText())) {
                    ...
                    /** A rontott index értéke 0, ezt még LEGALÁBB 2x ki fogja 
                     * kérdezni a körben(1000) ha a következö válasz jó, akkor 
                     * 1 lesz az értéke ha utána rontod, akkor újra 0 és megint
                     * 2x jól kell válaszolni, hogy hozzáadja az 
                     * (meg)tanultIndexek listához.
                     */
            ...
            // color word (second enter)
                ...
                if (!rs.isEmpty()) {
                    ...
                    if (ds.getUnlearned(keyListSize) > 100) {
                        ...
                        /** 
                         * 100 alatt csökkenti a futam kombobox tételeit,
                         * ha a kikérdezetlen szavak száma 60, akkor
                         * ne lehessen 100-as futamot választani, stb
                         
                        String kombo = ks.getCurrentKombo();
                        if (ds.getFutamComboBoxMaxIndex(kombo) > 1) {
                            if (ds.cutFutamComboBoxMaxIndex(keyListSize, kombo)) {
                                initFutamComboBox();
    ...
    /**
     * GYAKORLÀS - info from the race combo box item cutting.
     */
    @SuppressWarnings("unchecked")
    @Deprecated
    // <editor-fold defaultstate="collapsed" desc="info from the race combo box item cutting">
    private void raceComboBoxItemCuttingInfo(String item) {
        InfoService iS = new InfoService(lcs.getCurrentCombo(), item);
